...
export class User {
  ...
  async checkIfUserRegistered() {
    if (!this.session || !this.authName) throw new Error("Sesión no iniciada");
    ...
    return response.rows.length > 0 && response.rows[0].user === this.authName;
  }

  async registerUser(referrer = null) {
    if (!this.session || !this.authName) throw new Error("Sesión no iniciada");
    ...
    return this.session.signTransaction(
      {
        actions: [{
          ...
          data: {
            user: this.authName,
            referrer: referrer || this.authName
          }
        }]
      },
      {
  ...
  async stakeNFTs(asset_ids) {
    ...
    if (!isRegistered) await this.registerUser();
